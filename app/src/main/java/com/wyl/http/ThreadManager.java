...
/**
 * @author : wuchao5
 * @date : 3/26/21 2:36 PM
 * @desciption : 队列管理类 线程管理类   使用单例来实现
 */
public class ThreadManager {

  public static ThreadManager getInstance() {
    return Hold.threadManager;
  }
  ...
  private ThreadManager() {
    ...
    threadPoolExecutor = new ThreadPoolExecutor(3, 10, 15, TimeUnit.SECONDS,
        new ArrayBlockingQueue<>(4), new RejectedExecutionHandler() {
      ...
      while (true) {
        try {
          ...
          if (httpTask.getFailNum() < 3) {
            ...
            iHttpRequest.getCallBackListener().onFailed(null);
            // 重试了三次依然失败
            // Log.e("请求重试机制=============", "失败超过3次");
          }
        } catch (InterruptedException e) {
